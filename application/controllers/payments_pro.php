...
class Payments_pro extends CI_Controller 
{
	...
	{	
		...
		if($this->session->userdata('is_login') == TRUE)
		{
			...
			if($user_type == 3) //3 is Buyer
			{
				...
				$cc = $this->buyers->creditcards($user_id,true);
			...
			$PaymentDetails = array(
									'amt' => ''.$this->cart->total(), 							// Required.  Total amount of order, including shipping, handling, and tax.  
									'currencycode' => 'USD', 					// Required.  Three-letter currency code.  Default is USD.
									'itemamt' => ''.$this->cart->total(), 						// Required if you include itemized cart details. (L_AMTn, etc.)  Subtotal of items not including S&H, or tax.
			...
			{
				$Item	 = array(
									'l_name' => $itemd['name'], 						// Item Name.  127 char max.
									'l_desc' => 'test', 						// Item description.  127 char max.
									'l_amt' => $itemd['price'], 							// Cost of individual item.
									'l_number' => $itemd['id'], 						// Item Number.  127 char max.
									'l_qty' => $itemd['qty'], 							// Item quantity.  Must be any positive integer.  
			...
			if(!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK']))
			{
				$errors = array('errors'=>$PayPalResult['ERRORS'], 'result' => 0);

				$html_result = $this->load->view('buyer/buyer-payment-result',$data, TRUE);	
				$returnmessage = array( 'display' => $html_result  , 'status' => 0);
			...
			else
			{
				...
				$data_buyer_trans = array('u_id' => $user_id,
										  'bt_invoice' => $user_id.'-'.$this->rndm_strng('numeric',10),
										  'bt_type' => 'paypal',
				...
				foreach($carts as $itemd)
				{
					...
					$data_tras_detail = array('bt_id' => $bt_id,
											  'ic_id' => $itemd['id'], 
											  'btd_quan' => $itemd['qty'], 
											  'btd_amount' => $itemd['price']-$itemd['options']['ship_cost'], 
				...
				foreach($carts as $itemd)
				{
					$item_id =  $itemd['id']; //get product id, to get the owner of this product and sends notification
					$user_email = $this->users->email_info_from_product('ic_id',$item_id); //return email info using product id
					$to = "jamesan2k@gmail.com, ".$user_email;
	...
	function Do_direct_payment_demo()
	{
		...
		$CCDetails = array(
							...
							'expdate' => '102014', 							// Required.  Credit card expiration date.  Format is MMYYYY
